                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="live-preview">
                            ...
                            <div class="col-md-12 mt-3">
                                <div id="imagePreview">
                                    @if (!empty($productById->image))
                                        @foreach (explode(',', $productById->image) as $image)
                                            <div class="box_img">
                                                <img src="{{ $image }}" alt="{{ $image }}">
                                                <button class="btnDelete_image">x</button>
                                            </div>
                                        @endforeach
                                    @endif
                                </div>
                            ...
                            <div class="col-md-6 mt-3">
                                ...
                                <input type="text" class="form-control" value="{{ $productById->product_name }}"
                                    name="productName" id="productName" placeholder="Nhập tên sản phẩm...">
                            </div>
                            <div class="col-md-6 mt-3">
                                <label for="productPrice" class="form-label">Giá sản phẩm</label>
                                <input type="number" class="form-control" value="{{ $productById->price }}"
                                    id="productPrice" placeholder="Nhập giá sản phẩm...">
                            </div>
                                    ...
                                    @if ($listCategories->toArray())
                                        @foreach ($listCategories as $cate)
                                            <option value="{{ $cate->category_id }}"
                                                {{ $cate->category_id == $productById->category_id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-12 mt-3">
                                ...
                                <textarea name="productDescribe" placeholder="Nhập mô tả sản phẩm..." class="form-control" id="productDescribe"
                                    cols="30" rows="10">{{ $productById->describe }}</textarea>
                            </div>
                                    ...
                                    @foreach ($variants as $key => $variant)
                                        <div class="variant d-flex flex-wrap">
                                            <div class="mb-3 w-25" style="margin-right: 18px;">
                                                <input type="number" value="{{ $variant->size }}" name="productSize[]"
                                            ...
                                            <input type="button" onclick="removeElement(this)"
                                                class="btn btn-outline-danger" value="Xóa" style="height: 37px;">
                                        </div>
                                    @endforeach
                                @endif
                            </div>
                            ...
                            <div class="col-12">
                                <div class="text-start">
                                    ...
                                    <button type="button" class="btn btn-outline-success addVariant">Thêm phân
                                        loại</button>
                                    <button type="button" id="btnUpdateProduct" class="btn btn-primary">Cập nhật sản
                                        phẩm</button>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            imagePreview.addEventListener('click', function(e) {
                if (e.target.classList.contains('btnDelete_image')) {
                    e.target.closest('.box_img').remove();
                }
            });
            ...
            btnUpdateProduct.addEventListener('click', function(e) {
                ...
                if (files.length > 0) {
                    ...
                    var remainingImages = [];
                    document.querySelectorAll('#imagePreview .box_img > img').forEach(img => {
                        remainingImages.push(img.src);
                    });

                    formData.append('remainingImages', remainingImages.join(','));
                }

                $.ajax({
                    url: "{{ route('sneaker.updateSneaker', ['sneakerId' => $sneakerId]) }}",
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function(response) {
                        if (response.successUpdateProduct) {
                            Swal.fire({
                                position: "top-end",
                                icon: "success",
